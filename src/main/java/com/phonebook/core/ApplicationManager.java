...
public class ApplicationManager {
    ...
    public void init() {
        ...
        if (browser.equalsIgnoreCase("chrome")) {
            ...
            //options.addArguments("window-size=1920x1080");//установка разрешения экрана
            //options.addArguments("headless");//безголовый режим
            driver = new ChromeDriver();//если значение браузера только если мы передаем chromedriver как условие
        ...
        wait = new WebDriverWait(driver, Duration.ofSeconds(5));//явное ождание. Указываем конкретное условие чего мы ждем
